...
const CicdAnimation: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-4xl mx-auto py-12 px-6">
      <div className="relative w-full aspect-[4/1] bg-slate-900/50 rounded-2xl border border-slate-800 overflow-hidden flex items-center justify-around px-12 group shadow-inner">
        {/* Connection Line */}
        <div className="absolute top-1/2 left-24 right-24 h-0.5 bg-slate-800 -translate-y-1/2 z-0" />
        <div 
        ...
        {steps.map((step, index) => (
          <div key={step.name} className="relative z-10 flex flex-col items-center gap-4">
            ...
            <div className="flex flex-col items-center">
              <span className={`text-[10px] font-black tracking-[0.2em] mb-1 ${activeStep === index ? step.color : 'text-slate-600'}`}>
                {step.name}
              </span>
              <div className={`h-1 w-1 rounded-full ${activeStep === index ? 'bg-emerald-500' : 'bg-slate-800'}`} />
            </div>
          </div>
        ))}
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-center animate-fade-in">
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em] h-4">
      ...
      <div className="mt-8 grid grid-cols-3 gap-8 w-full">
        {steps.map((step, index) => (
          <div key={index} className={`p-4 rounded-xl border transition-all duration-500 ${activeStep === index ? 'bg-slate-900/50 border-emerald-500/50' : 'border-transparent opacity-50'}`}>
             <div className="text-[10px] font-mono text-slate-500 mb-2">0{index + 1}</div>
